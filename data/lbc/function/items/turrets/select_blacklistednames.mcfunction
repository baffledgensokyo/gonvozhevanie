...
$execute if block ~ ~ ~ barrel{Items:[{Slot:$(i)b,components:{"minecraft:custom_name":"\"_\""}}]} as @e[type=#mobs,type=!player,distance=..$(dist)] unless data entity @s CustomName run tag @s add blacklisted
