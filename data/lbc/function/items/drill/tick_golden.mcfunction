...
execute if score tmp lbc.math matches 4.. run scoreboard players set if lbc.math 1
execute if items entity @s container.* redstone_block if score tmp lbc.math matches ..4 run scoreboard players set if lbc.math 2
...
execute if score if lbc.math matches 1..3 unless items entity @s weapon.mainhand *[minecraft:damage=76] anchored eyes positioned ^ ^ ^ run function lbc:items/drill/tick_golden_loop
execute if score if lbc.math matches 1..3 run playsound item.trident.riptide_1 block @a ~ ~ ~ 0.5 2

advancement revoke @s only lbc:golden_drill_use